...
def save_uploaded_file(file_content, file_name):
    """
    Yüklenen dosyayı geçici bir dizine kaydedin
    """
    import tempfile
    import os
    import uuid
...
def app():
    ...
    st.title("Vektör Verilerini Yükle")
    ...
    with row1_col2:
        ...
        backend = st.selectbox(
            "Bir Harita Sistemi Seçin", ["folium", "kepler.gl", "pydeck"], index=2
        )
        ...
        if data or url:
            ...
            with row1_col1:
                ...
                if backend == "pydeck":
                    ...
                    random_column = None
                    with container:
                        random_color = st.checkbox(
                            "Rastgele renkler uygula", True)
                        if random_color:
